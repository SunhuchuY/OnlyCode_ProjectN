...
public class PowerOffScreen : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        curWaveText.text = $"현재 웨이브 : {GameManager.Instance.userDataManager.userData.Wave}";

        bloodstoneText.text = $"{GameManager.Instance.userDataManager.userData.MagicStone}";
        goldText.text = $"{GameManager.Instance.userDataManager.userData.Gold}";
