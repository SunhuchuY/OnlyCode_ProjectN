...
public class StatisticScreen : MonoBehaviour
{
    ...
    {
        if(GameManager.Instance.userDataManager.Magicstone >= requiredBloodStone)
        {
            GameManager.Instance.userDataManager.Magicstone += -requiredBloodStone;

            if (GameManager.RandomRange(goldRange_Start, goldRange_End))
            {
                GameManager.Instance.userDataManager.Gold += 100;
            }
        }
    }
}
