...
public class GambleSystem : MonoBehaviour
{
    ...
    private void InsertCardRateDict()
    {
        cardRateDict.Add(cardRateEnum.R, new List<int>());
        cardRateDict.Add(cardRateEnum.SR, new List<int>());
        cardRateDict.Add(cardRateEnum.SSR, new List<int>());

        foreach (var item in GameManager.Instance.skillTreeManager.CardDataContainer.cards)
        {
            cardRateDict[item.cardRate].Add(item.cardIndex);
        }
    }
    ...
    public void GamebleSystem_Start()
    {
        ...
        for (int i = 0; i < curnumofCard; i++)
        {
            ...
            {
                Debug.LogError($"IndexOutOfRange: {ex.Message}, Length is {GameManager.Instance.skillTreeManager.CurCardStates.Length} but you try index {cardScript.cardIndex}");
            }
    ...
    {
        int rand = Random.Range(0, cardRateDict[rate].Count);
        return cardRateDict[rate].ElementAt(rand);
    }
